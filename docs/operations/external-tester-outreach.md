...
summary: Refresh the unpublished launch packet to v0.17.2 while keeping tester authority unassigned
---
...
> differed from the host's native format. Release `v0.17.2` is published and
...
The venue copy below references the current `v0.17.2` release as an
...
- [x] Publish immutable `v0.17.2` with RPM, DEB, Arch, CCS, checksums, and the
...
- [x] Publish and independently verify the synchronized `v0.17.2` suite that
