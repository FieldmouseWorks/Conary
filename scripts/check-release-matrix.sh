...
remi_deployment_action=".github/actions/deploy-remi-bundle/action.yml"
remi_completion_policy=".github/actions/deploy-remi-bundle/resolve-completion.py"
remi_completion_tests="scripts/test-remi-deployment-completion.py"
remi_wait_tests="scripts/test-remi-repopulation-wait.py"
...
resolution_survey_request="scripts/remi-survey-request.py"
...
required_files=(
    ...
    "$remi_deployment_action"
    "$remi_completion_policy"
    "$remi_completion_tests"
    "$remi_wait_tests"
...
for workflow in .github/workflows/*.yml "$remi_deployment_action"; do
...
require_job_match "$deploy_workflow" deploy-remi 'name: Deploy remi bundle[\s\S]*uses: \./workflow-authority/\.github/actions/deploy-remi-bundle[\s\S]*completion-mode: launch-authority' 'release deploy uses launch-aware shared completion authority'
require_job_match "$deploy_workflow" deploy-remi 'bundle_dir="source-artifacts/\$\{BUNDLE_NAME\}"[\s\S]*sha256sum -c SHA256SUMS[\s\S]*bundle="\$\{bundle_dir\}/remi-\$\{VERSION\}-linux-x64\.tar\.gz"[\s\S]*binary_sha256="\$\(tar xOzf "\$bundle"[\s\S]*binary-sha256: \$\{\{ steps\.remi-bundle\.outputs\.binary_sha256 \}\}' 'Remi deployment must verify the complete suite checksums before staging its bundle'
require_match "$remi_deployment_action" 'curl -fsS --max-time 30 https://remi\.conary\.io/health[\s\S]*if \[\[ "\$COMPLETION_MODE" == "active-repopulation" \]\]; then[\s\S]*\.ready == true' 'shared Remi deployment liveness and mode-specific readiness proof'

require_job_match "$candidate_deploy_workflow" deploy-remi-candidate 'timeout-minutes: 300[\s\S]*uses: \./workflow-authority/\.github/actions/deploy-remi-bundle[\s\S]*completion-mode: \$\{\{ github\.event\.inputs\.completion_mode \}\}' 'candidate deploy forwards its explicit mode to shared completion'
require_job_match "$deploy_workflow" deploy-remi 'timeout-minutes: 300[\s\S]*commit-sha: \$\{\{ steps\.remi-bundle\.outputs\.commit_sha \}\}' 'release deploy binds shared completion to its exact tag commit'
require_match "$remi_deployment_action" 'git show "\$\{WORKFLOW_SHA\}:deploy/remi-deploy-helper\.sh" > "\$workflow_deploy_helper"[\s\S]*sha256sum "\$workflow_deploy_helper"[\s\S]*scp "\$\{ssh_opts\[@\]\}" "\$workflow_deploy_helper"' 'shared deployment installs the exact workflow helper independently of artifact history'
require_match "$remi_deployment_action" 'git show "\$\{WORKFLOW_SHA\}:docs/roadmaps/launch-status\.json"[\s\S]*resolve-completion\.py[\s\S]*helper_args=\(wait-remi-repopulation 3600\)[\s\S]*state: "not_applicable", reason: "no_active_universe"' 'shared completion retains exact launch authority and typed repopulation applicability'
require_match "$remi_deployment_action" '\.repopulation_wait\.budget_ms == 3600000[\s\S]*\.repopulation_wait\.elapsed_ms <= \.repopulation_wait\.budget_ms[\s\S]*\.repopulation_wait\.attempts\[-1\]\.operation == "ingress"' 'active completion requires bounded final ingress evidence'
forbid_match "$remi_deployment_action" 'seq 1 120|sleep 30' 'workflow attempt-count repopulation loop'
...
require_match "$remi_deployment_action" 'ssh_opts=\(-F "\$REMI_SSH_CONFIG"\)[\s\S]*ssh "\$\{ssh_opts\[@\]\}"[\s\S]*scp "\$\{ssh_opts\[@\]\}"[\s\S]*ssh "\$\{ssh_opts\[@\]\}" "\$target" bash -s' 'candidate deploy must route every remote operation through the pinned production SSH configuration'
require_match "$remi_deployment_action" 'scp "\$\{ssh_opts\[@\]\}" "\$BUNDLE"[\s\S]*inspect-remi-candidate-baseline[\s\S]*\$VERSION[\s\S]*\$BINARY_SHA256[\s\S]*\$remote_bundle[\s\S]*> remi-predeployment-inspection\.json[\s\S]*baseline_status=\$\?[\s\S]*jq -e -f deploy/remi-predeployment-inspection\.jq[\s\S]*\.measurement\.output_bytes == \$baseline_bytes[\s\S]*failure_phase: "predeployment-candidate-baseline"' 'candidate deploy verifies the exact staged candidate, validates the schema-compatible live baseline, and retains typed preflight failure evidence before mutation'
...
require_match "$remi_deployment_action" 'deploy-remi[\s\S]*"\$1" "\$deployed_binary_sha256" "\$2" "\$3" "\$4"[\s\S]*verify-ingress[\s\S]*start_phase private-candidate-inspection[\s\S]*verify-ingress[\s\S]*capture_completion_inspection "\$requirement"' 'candidate deploy verifies static ingress after mutation and completion'
require_match "$remi_deployment_action" 'if \[\[ "\$COMPLETION_MODE" == "active-repopulation" \]\]; then[\s\S]*\.ready == true[\s\S]*ready_status=.*curl[\s\S]*"200" \|\| "\$ready_status" == "503"[\s\S]*\.ready \| type == "boolean"' 'candidate deploy mode-specific public readiness contract'
require_match "$remi_deployment_action" 'set \+e[\s\S]*ssh "\$\{ssh_opts\[@\]\}" "\$target" bash -s[\s\S]*> remi-deployment-inspection\.json <<.REMOTE_EOF.[\s\S]*deployment_inspection_is_typed\(\)[\s\S]*capture_completion_inspection\(\)[\s\S]*helper_args=\(inspect-remi "\$requirement"\)[\s\S]*"\$\{helper_args\[@\]\}" 2>/dev/null[\s\S]*inspection_failure=predicate-unsatisfied[\s\S]*inspection_failure=command-failed[\s\S]*inspection_failure=invalid-typed-output[\s\S]*emit_captured_inspection_if_typed[\s\S]*deploy_status=\$\?[\s\S]*latest_refresh\.run_id[\s\S]*latest_refresh\.redactions[\s\S]*exit "\$deploy_status"' 'candidate deploy retains one validated final typed inspection with channel-separated diagnostics'
forbid_match "$remi_deployment_action" '"\$\{helper_args\[@\]\}" 2>&1' 'candidate deployment inspection JSON mixed with stderr diagnostics'
require_match "$remi_deployment_action" 'deployment_evidence_schema_version: 3[\s\S]*repository_refreshes[\s\S]*start_phase database-transition-and-restart[\s\S]*start_phase ingress-after-transition[\s\S]*start_phase forced-refresh-all[\s\S]*start_phase "forced-refresh-\$\{profile\}"[\s\S]*start_phase private-candidate-inspection[\s\S]*start_phase ingress-after-completion[\s\S]*failure_phase: "remote-session-or-transport"[\s\S]*\.deployment\.outcome == \$expected_outcome[\s\S]*\.deployment\.phases[\s\S]*\.duration_ms >= 0' 'candidate deploy retains typed refresh generations, phase timing, and early-failure evidence'
require_match "$remi_deployment_action" 'inspect-remi-storage[\s\S]*> remi-predeployment-storage\.json[\s\S]*\.filesystem\.available_bytes[\s\S]*\.database\.logical_bytes[\s\S]*\.database\.allocated_bytes[\s\S]*\.transition_backups\.directories[\s\S]*> remi-deployment-storage\.json[\s\S]*Storage evidence \(before -> after\)[\s\S]*remi-deployment-storage\.json[\s\S]*remi-predeployment-storage\.json' 'candidate deploy retains before-and-after numeric storage evidence'
require_literal_count "$remi_deployment_action" "storage_evidence_jq='" 1 'candidate deploy has one storage-evidence predicate authority'
require_literal_count "$remi_deployment_action" 'jq -e "$storage_evidence_jq"' 2 'candidate deploy reuses one storage-evidence predicate before and after deployment'
require_match "$remi_deployment_action" 'name: Summarize final typed deployment inspection[\s\S]*if: \$\{\{ always\(\) \}\}[\s\S]*latest_refresh\.failure_stage[\s\S]*latest_refresh\.failure_category[\s\S]*latest_refresh\.failure_evidence_sha256' 'candidate deploy summarizes sanitized refresh failure authority'
require_match "$remi_deployment_action" 'name: Upload final sanitized deployment inspection[\s\S]*if: \$\{\{ always\(\) \}\}[\s\S]*uses: actions/upload-artifact@bbbca2ddaa5d8feaa63e36b76fdaad77386f024f[\s\S]*remi-candidate-manifest\.json[\s\S]*remi-deployment-inspection\.json[\s\S]*remi-predeployment-inspection\.json[\s\S]*retention-days: 30' 'candidate deploy retains before-and-after sanitized inspection artifacts plus source provenance'
...
require_match "$resolution_survey_workflow" 'workflow_dispatch:[\s\S]*oracle_run_id:[\s\S]*required: true[\s\S]*permissions:[\s\S]*actions: read[\s\S]*contents: read' 'resolution survey exact oracle input, read-only permissions, and shared serialization'
...
require_job_match "$resolution_survey_workflow" survey 'Download exact export and deployment evidence[\s\S]*remi-resolution-survey-transport\.py build-input[\s\S]*--workflow-commit "\$CHAIN_WORKFLOW_SHA"[\s\S]*--assembly-evidence[\s\S]*--lane "fedora-44=[\s\S]*--lane "ubuntu-26\.04=[\s\S]*--lane "arch=[\s\S]*git merge-base --is-ancestor "\$deployed_commit" origin/main[\s\S]*git merge-base --is-ancestor "\$deployed_commit" "\$producer_commit"[\s\S]*git merge-base --is-ancestor "\$producer_commit" origin/main' 'resolution survey downloads and authenticates every exact current-operator assembled input'
require_job_match "$resolution_survey_workflow" survey 'Bind the exact survey or retained recovery request[\s\S]*actions/runs/\$\{RETAINED_SURVEY_RUN_ID\}/attempts/\$\{RETAINED_SURVEY_RUN_ATTEMPT\}[\s\S]*remi-survey-request\.py[\s\S]*--operation "\$OPERATION" --repository "\$GITHUB_REPOSITORY"[\s\S]*--workflow-commit "\$WORKFLOW_SHA"[\s\S]*--workflow-run-id "\$GITHUB_RUN_ID" --workflow-run-attempt "\$GITHUB_RUN_ATTEMPT"[\s\S]*--oracle-run-id "\$ORACLE_RUN_ID"[\s\S]*--retained-survey-run-id "\$RETAINED_SURVEY_RUN_ID"[\s\S]*--retained-survey-run-attempt "\$RETAINED_SURVEY_RUN_ATTEMPT"[\s\S]*git merge-base --is-ancestor "\$chain_workflow_commit" origin/main[\s\S]*CHAIN_WORKFLOW_SHA: \$\{\{ steps\.request\.outputs\.chain_workflow_commit \}\}[\s\S]*REQUEST_SURVEY_ID: \$\{\{ steps\.request\.outputs\.survey_id \}\}[\s\S]*survey_id="\$REQUEST_SURVEY_ID"' 'resolution survey exact original attempt and authenticated request handoff'
require_job_match "$resolution_survey_workflow" survey 'conary-remi-deploy install-helper[^\n]*\n[ \t]*if \[\[ "\$OPERATION" == recover \]\]; then[\s\S]*fetch_verified_recovery\n[ \t]*jq -e .*authority == "diagnostic_only" and \.availability == "retained"[\s\S]*and \.input_binding == "verified"[\s\S]*service_operation:"none",original_survey_outcome:"unchanged"[\s\S]*echo "recovery_outcome=retrieved"[\s\S]*exit 0\n[ \t]*fi\n[ \t]*scp .*"\$INPUT_TRANSPORT"' 'resolution survey recovery requires verified retained input and exits before survey staging'
...
require_match "$remi_deploy_helper" 'basis=3540[\s\S]*last_ready_duration_seconds[\s\S]*basis="\$previous"[\s\S]*basis > 0 \? basis : 1\) \* 2[\s\S]*budget <= 7200[\s\S]*timeout "\$budget"[\s\S]*remaining_ms=\$\(\(budget_ms - elapsed_ms\)\)[\s\S]*"\$HEALTH_URL"[\s\S]*restart_to_ready_seconds:\$ready' 'Remi restore budget derives from recorded startup evidence with a hard ceiling'
...
require_job_match "$resolution_survey_workflow" survey 'record_helper_evidence\(\)[\s\S]*then "helper_failed" else "helper_succeeded"[\s\S]*status:\$status[\s\S]*message:[\s\S]*fetch_verified_recovery\(\)[\s\S]*conary-remi-deploy export-resolution-survey-evidence[\s\S]*verify-recovery[\s\S]*recover_helper_failure\(\)[\s\S]*fetch_verified_recovery \|\| true[\s\S]*record_helper_evidence "\$observed_status" "\$workflow_status" "\$recovery_state"[\s\S]*recover_helper_failure "\$status"[\s\S]*rm -f -- "\$key"[\s\S]*helper_status=0' 'resolution survey recovers any helper failure before SSH cleanup independently of its report'
