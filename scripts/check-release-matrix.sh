...
require_match "$resolution_survey_transport" 'def verify_recovery[\s\S]*exact_positive_int\(item\["size"\], "survey recovery file size"\)[\s\S]*"private_host_path", "empty", "redaction_unproven"' 'resolution survey recovery requires positive included sizes and typed withheld reasons'
