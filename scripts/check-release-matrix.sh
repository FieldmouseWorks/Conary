...
require_job_match "$resolution_survey_workflow" survey 'sanitize_helper_stderr\(\)[\s\S]*python3 scripts/remi-survey-ssh-diagnostic\.py \\\n[ \t]*--stderr "\$helper_stderr" --ssh-config "\$REMI_SSH_CONFIG" \\\n[ \t]*--known-hosts "\$known_hosts"[\s\S]*sanitizer_failed[\s\S]*--argjson stderr "\$\(sanitize_helper_stderr\)"[\s\S]*stderr:\$stderr' 'resolution survey sanitizes connection identifiers before constructing failure artifacts'
...
require_match 'scripts/remi-survey-ssh-diagnostic.py' 'def public_diagnostic[\s\S]*if remaining_identifier\(message, identifiers\):[\s\S]*withheld\("connection_identifier_remaining"\)[\s\S]*def sanitize[\s\S]*IP_LITERAL.sub[\s\S]*return public_diagnostic\(message, identifiers\)' 'resolution survey withholds residual connection identifiers after redaction'
...
require_match "$resolution_survey_transport" 'def verify_recovery[\s\S]*validate_input_evidence[\s\S]*authority[\s\S]*diagnostic_only[\s\S]*input_sha256[\s\S]*copy_tar_member[\s\S]*forbid_recovery_host_paths[\s\S]*retained recovery input bytes differ from authenticated input' 'resolution survey recovery checks exact identities digests and input binding without survey authority'
require_match "$remi_deploy_helper" 'export_resolution_survey_evidence\(\)[\s\S]*size == 0[\s\S]*reason:"empty"[\s\S]*continue[\s\S]*jq -en --stream --arg pattern "\$SURVEY_HOST_PATH_PATTERN"[\s\S]*reason:"private_host_path"[\s\S]*reason:"invalid_json"[\s\S]*members\+=' 'resolution survey recovery withholds empty and unsafe JSON before archiving'
require_match "$resolution_survey_transport" 'def verify_recovery[\s\S]*exact_positive_int\(item\["size"\], "survey recovery file size"\)[\s\S]*"private_host_path", "empty", "invalid_json"' 'resolution survey recovery requires positive included sizes and typed withheld reasons'
