...
cases = (
    ...
    ('test_check_release_matrix_rejects_omitted_active_ingress_proof', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '.repopulation_wait.attempts[-1].operation == "ingress"', '.repopulation_wait.attempts[-1].operation == "inspect"', 'active completion requires bounded final ingress evidence'),
    ('test_check_release_matrix_rejects_candidate_launch_policy_override', 'replace', '.github/workflows/deploy-remi-candidate.yml', '          completion-mode: ${{ github.event.inputs.completion_mode }}', '          completion-mode: launch-authority', 'candidate deploy forwards its explicit mode to shared completion'),
    ('test_check_release_matrix_rejects_untyped_recovery_scalar', 'replace', 'deploy/remi-deploy-helper.sh', 'if ($contract.types | index($type)) == null then "type_mismatch"', 'if false then "type_mismatch"', 'resolution survey recovery validates scalar and container types from one schema'),
    ('test_check_release_matrix_rejects_unknown_recovery_scalar', 'replace', 'deploy/remi-deploy-helper.sh', 'if $contract == null then "unknown_key"', 'if $contract == null then null', 'resolution survey recovery validates scalar and container types from one schema'),
    ('test_check_release_matrix_rejects_untyped_recovery_array', 'replace', 'deploy/remi-deploy-helper.sh', 'if .types | index("array") then .items else null end', 'if .types | index("array") then object_value else null end', 'resolution survey recovery validates scalar and container types from one schema'),
    ('test_check_release_matrix_rejects_unchecked_recovery_container', 'replace', 'deploy/remi-deploy-helper.sh', 'range(0; $path | length) as $index', 'range(0; 0) as $index', 'resolution survey recovery validates scalar and container types from one schema'),
    ('test_check_release_matrix_rejects_missing_recovery_worker_schema', 'replace', 'deploy/remi-deploy-helper.sh', '"workers", "memory_budget_bytes", "measured_worker_rss_bytes"', '"memory_budget_bytes", "measured_worker_rss_bytes"', 'resolution survey recovery streams typed survey diagnostics into sanitized archive members'),
    ('test_check_release_matrix_rejects_unsanitized_recovery_details', 'replace', 'deploy/remi-deploy-helper.sh', 'if survey_sanitize_json_stream "$file"', 'if cat "$file"', 'resolution survey recovery streams typed survey diagnostics into sanitized archive members'),
    ('test_check_release_matrix_rejects_two_argument_recovery_doc', 'replace', 'docs/operations/infrastructure.md', 'export-resolution-survey-evidence <survey-id> <export-id> <input-manifest-sha256>', 'export-resolution-survey-evidence <survey-id> <export-id>', 'resolution survey recovery documentation matches the helper usage contract'),
    ('test_check_release_matrix_rejects_drifted_recovery_usage', 'replace', 'deploy/remi-deploy-helper.sh', 'export-resolution-survey-evidence <survey-id> <export-id> <input-manifest-sha256>', 'export-resolution-survey-evidence <survey-id> <export-id> <input-manifest-sha256> <new-required-argument>', 'resolution survey recovery documentation matches the helper usage contract'),
    ...
    ('test_check_release_matrix_rejects_missing_completed_restore_key', 'replace', 'deploy/remi-deploy-helper.sh', 'retained:object_of(["kind", "id"])', 'retained:object_of(["kind"])', 'resolution survey recovery admits the completed restore envelope vocabulary'),
    ('test_check_release_matrix_rejects_missing_completed_restore_kind', 'replace', 'deploy/remi-deploy-helper.sh', 'kind:(enumeration(["completed_resolution_survey"', 'kind:(enumeration(["obsolete"', 'resolution survey recovery admits the completed restore envelope vocabulary'),
    ...
    ('test_check_release_matrix_rejects_unbound_recovery_digest', 'replace', 'deploy/remi-deploy-helper.sh', '"candidate_manifest_sha256", "source_sha256", "sha256"', '"candidate_manifest_sha256", "source_sha256", "message"', 'resolution survey safe scalar strings require their owning fields'),
    ('test_check_release_matrix_rejects_unbound_recovery_decimal', 'replace', 'deploy/remi-deploy-helper.sh', 'run_id:strings("decimal")', 'message:strings("decimal")', 'resolution survey safe scalar strings require their owning fields'),
    ('test_check_release_matrix_rejects_unbound_recovery_timestamp', 'replace', 'deploy/remi-deploy-helper.sh', '["timestamp", "started_at", "completed_at"]', '["message", "started_at", "completed_at"]', 'resolution survey safe scalar strings require their owning fields'),
    ...
    ('test_check_release_matrix_rejects_fabricated_empty_stderr_message', 'replace', '.github/workflows/survey-remi-resolution.yml', 'if $stderr | has("message") then {message:$stderr.message} else {} end', '{message:($stderr.message // "malformed evidence")}', 'resolution survey separates empty stderr from report evidence failures'),
    ...
    ('test_check_release_matrix_rejects_all_profile_retry', 'replace', '.github/actions/deploy-remi-bundle/action.yml', 'refresh?force=true&profile=${profile}', 'refresh?force=true', 'retries only exact failed public profiles'),
    ...
    ('test_check_release_matrix_rejects_candidate_checkout_fencing_policy', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '            -f "$workflow_fencing_policy" \\', '            -f deploy/remi-postdeployment-fencing.jq \\', 'private candidate deploy evaluates post-transition fencing from the exact workflow authority, independent of the candidate checkout'),
    ('test_check_release_matrix_rejects_candidate_completion_catalog_rescan', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '.candidate_verification.catalog_bytes_hashed == 0', '.candidate_verification.catalog_bytes_hashed >= 0', 'candidate deploy binds one causal bounded private-candidate inspection to the exact transition while retaining full active inspection'),
    ('test_check_release_matrix_rejects_candidate_tier_as_public_refresh_authority', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '                  | select(. != "solus")]', '                  | select(true)]', 'retries only exact failed public profiles'),
    ...
    ('test_check_release_matrix_rejects_discarded_candidate_failure_inspection', 'replace', '.github/actions/deploy-remi-bundle/action.yml', "          > remi-deployment-inspection.json <<'REMOTE_EOF'", "          > /dev/null <<'REMOTE_EOF'", 'candidate deploy retains one validated final typed inspection'),
    ...
    ('test_check_release_matrix_rejects_literal_ssh_target_fallback', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '        target="$REMI_SSH_TARGET"', '        target="${REMI_SSH_TARGET:-operator@ssh.example.test}"', 'literal user@host fallback'),
    ...
    ('test_check_release_matrix_rejects_missing_candidate_failure_artifact', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '      uses: actions/upload-artifact@bbbca2ddaa5d8feaa63e36b76fdaad77386f024f # v7.0.0', '      run: echo "deployment inspection upload removed"', 'candidate deploy retains before-and-after sanitized inspection artifacts'),
    ('test_check_release_matrix_rejects_missing_candidate_phase_evidence', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '          start_phase database-transition-and-restart', '          echo "database transition timing removed" >&2', 'candidate deploy retains typed refresh generations, phase timing, and early-failure evidence'),
    ('test_check_release_matrix_rejects_missing_candidate_storage_evidence', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '          > remi-predeployment-storage.json', '          > /dev/null', 'candidate deploy retains before-and-after numeric storage evidence'),
    ('test_check_release_matrix_rejects_missing_shared_candidate_storage_predicate', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '          || ! jq -e "$storage_evidence_jq" \\\n            remi-deployment-storage.json >/dev/null; then', '          || ! jq -e "true" \\\n            remi-deployment-storage.json >/dev/null; then', 'candidate deploy reuses one storage-evidence predicate before and after deployment'),
    ...
    ('test_check_release_matrix_rejects_missing_post_deploy_remi_readiness', 'replace', '.github/actions/deploy-remi-bundle/action.yml', "jq -e '.ready == true'", "jq -e 'true'", 'shared Remi deployment liveness and mode-specific readiness proof'),
    ('test_check_release_matrix_rejects_missing_refresh_causal_floor', 'replace', '.github/actions/deploy-remi-bundle/action.yml', 'refresh?force=true&accept_completed_after=${transition_completed_at}', 'refresh?force=true', 'private candidate deploy coalesces one bounded post-transition refresh'),
    ...
    ('test_check_release_matrix_rejects_mixed_candidate_inspection_channels', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '              "${helper_args[@]}" 2>/dev/null)"; then', '              "${helper_args[@]}" 2>&1)"; then', 'channel-separated diagnostics'),
    ...
    ('test_check_release_matrix_rejects_unbound_candidate_binary', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '          and .deployment.binary_sha256 == $expected_binary', '          and (.deployment.binary_sha256 | type == "string")', 'candidate deploy binds final evidence to exact commit and binary'),
    ('test_check_release_matrix_rejects_unbound_candidate_deploy_digest', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '            "$1" "$deployed_binary_sha256" "$2" "$3" "$4" >&2', '            "$1" "$2" "$2" "$3" "$4" >&2', 'candidate deploy verifies static ingress after mutation and completion'),
    ...
    ('test_check_release_matrix_rejects_unbounded_candidate_completion_inspection', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '--accept-candidates-completed-after "$completed_after"', '--config "$completed_after"', 'candidate deploy binds one causal bounded private-candidate inspection to the exact transition while retaining full active inspection'),
    ...
    ('test_check_release_matrix_rejects_untyped_refresh_coalescing_result', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '                and (.coalesced | type == "boolean")', '                and true', 'private candidate deploy coalesces one bounded post-transition refresh'),
    ...
    ('test_check_release_matrix_rejects_unvalidated_candidate_inspection', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '          deployment_inspection_is_typed() {', '          deployment_inspection_is_untyped() {', 'channel-separated diagnostics'),
    ...
    ('test_check_release_matrix_rejects_unverified_remi_suite_candidate', 'replace', '.github/workflows/deploy-and-verify.yml', '          binary-sha256: ${{ steps.remi-bundle.outputs.binary_sha256 }}', '          binary-sha256: ${{ needs.resolve.outputs.version }}', 'Remi deployment must verify the complete suite checksums before staging its bundle'),
    ...
    ('test_check_release_matrix_rejects_wrong_candidate_inspection_predicate', 'replace', '.github/actions/deploy-remi-bundle/action.yml', '              requirement=--require-private-candidates', '              requirement=--require-repopulated', 'candidate deploy mode-specific typed inspection predicate'),
...
main() {
    ...
    python3 "$REPO_ROOT/scripts/test-remi-deployment-completion.py"
    python3 "$REPO_ROOT/scripts/test-remi-repopulation-wait.py"
    run_release_policy_mutation_cases
}
