...
pub enum StaticFixture {
    ...
    Shell,
    /// The hermetic `/sbin/init` + boot-asset base (`conary-test-base`).
    #[serde(rename = "conary-test-base")]
    Base,
}
...
impl StaticFixture {
    ...
    /// before any fixture whose hooks or init need it. This is the single source
    /// for that order: callers iterate it instead of re-spelling a separate
    /// list.
    pub const ALL: &[StaticFixture] = &[Self::Shell, Self::Base];
