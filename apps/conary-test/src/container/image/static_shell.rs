...
//! Resolving and validating the host static binary the hermetic provider
//! fixtures stage as `/bin/sh` and `/sbin/init`, plus the `/bin/sh` functional
//! probe.
...
/// Which hermetic provider fixtures the selected suites install.
///
/// Each provider's payload is copied from the host, so resolving a host binary
/// must be a deliberate consequence of the manifests an image is built for, not
/// an unconditional image-build prerequisite. `NotInstalled` lets an image
/// build for suites that never install a provider without any host binary
/// present.
...
pub enum ShellProviderRequirement {
    ...
    Shell,
    /// A selected suite installs only the `/sbin/init` base provider.
    Base,
...
impl ShellProviderRequirement {
    ...
    ///
    /// Command text is deliberately ignored: a quoted or rewritten install
    /// command still opts an image into the provider by declaring the fixture,
    /// and a command that merely mentions the variable does not.
    pub fn from_manifests<'a>(manifests: impl IntoIterator<Item = &'a TestManifest>) -> Self {
        ...
        match (shell, base) {
            ...
            (true, true) => Self::ShellAndBase,
        }
    }

    /// Whether a selected suite installs the `/bin/sh` provider.
    pub(super) fn shell_required(&self) -> bool {
        matches!(self, Self::Shell | Self::ShellAndBase)
    }

    /// Whether a selected suite installs the `/sbin/init` base provider.
    pub(super) fn base_required(&self) -> bool {
        matches!(self, Self::Base | Self::ShellAndBase)
    }
}
...
/// Locate the static binary the hermetic provider fixtures stage from the host.
///
...
/// is not enough: a dynamic busybox or a script would be signed as the provider
/// payload and then fail inside the otherwise-empty selected-root chroot, so
/// the ELF is inspected before staging.
pub(super) fn resolve_static_test_binary() -> Result<PathBuf> {
    resolve_static_candidate(&accept_any_static_binary)
}

/// Locate the static shell the `/bin/sh` provider stages and prove it can run
/// the fixture hooks' external commands.
///
/// The `/sbin/init` base provider stages the same host binary but never runs
/// fixture hooks, so it uses [`resolve_static_test_binary`] directly and skips
/// the shell probe.
pub(super) fn resolve_static_test_shell() -> Result<PathBuf> {
    resolve_static_candidate(&probe_static_shell)
}

/// Accept any already-validated static binary: the base provider runs no hooks.
fn accept_any_static_binary(_: &Path) -> std::result::Result<(), StaticShellError> {
    Ok(())
}

/// Search the host for a static candidate, confirming each validated one with
/// `confirm` before selecting it.
fn resolve_static_candidate(
    confirm: &dyn Fn(&Path) -> std::result::Result<(), StaticShellError>,
) -> Result<PathBuf> {
    ...
    if let Some(path_var) = std::env::var_os("PATH") {
        ...
        for directory in std::env::split_paths(&path_var) {
            ...
            match validate_static_shell(&candidate).and_then(|()| confirm(&candidate)) {
    ...
    bail!(
        "integration provider fixtures need a statically linked binary: install busybox (static) or set {STATIC_TEST_SHELL_ENV}=<path>"
    )
}
